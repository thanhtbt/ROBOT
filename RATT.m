...
while ii <= T
    ...
    for ll = 1 : window
        ...
        H_Omega    = H_t(idx_t,:);
        x_Omega    = x_t_ll(idx_t);

        [o_t,~,~]  = outlier_detection(x_Omega,H_Omega,[]);
        idx_ot      = find(abs(o_t) < 1);
        Omega_ot_ll = ones(size(H_Omega,1),1);
        Omega_ot_ll(~idx_ot) = 0;
        Omega_ot    = ones(size(H_t,1),1);
        Omega_ot(idx_t,:) = Omega_ot_ll;
        
        
        g4         = H_Omega(idx_ot,:) \ x_Omega(idx_ot,:);     
        G4         = [G4 g4];
         
        X_t_ll_re  = H_t * g4;
        X_t_ll_re  = tensor(reshape(X_t_ll_re,[tt_dim(1:end-1)]));
             
        Omega_Ot     = tensor(reshape(Omega_ot,[tt_dim(1:end-1)]));
         
        Delta_X_t_ll = Omega_t_ll .* Omega_Ot .* (X_t_ll - X_t_ll_re);
