...
outlier_den  = [0.1 0.5 0.9];


PER_ATT  = zeros(length(outlier_den),tt_dim(end));

for ll = 1 : length(outlier_den)
    
    outlier_den_ll = outlier_den(ll);
    fprintf('outlier density %f \n',outlier_den_ll);
...
lgd = legend([d12,d22,d32],'$\omega_{outlier} = 10\%$','$\omega_{outlier} = 50\%$','$\omega_{outlier} = 90\%$');
lgd.FontSize = 24;
