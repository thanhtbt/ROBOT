...
outlier_den  = 0.2;
outlier_fac  = [0.1 0.7 1];


PER_ATT  = zeros(length(outlier_fac),tt_dim(end));

for ll = 1 : length(outlier_fac)
    
    outlier_fac_ll = outlier_fac(ll);
    fprintf('outlier intensity %f \n',outlier_fac_ll);
    ...
    for ii = 1 : n_exp
        ...
        O(p(1:L))   = outlier_fac_ll * rand(L,1);
...
lgd = legend([d12,d22,d32],'$\texttt{fac-outlier} = 0.1$','$\texttt{fac-outlier} = 1$','$\texttt{fac-outlier} = 10$');
lgd.FontSize = 24;
